...
namespace NinjaTrader.NinjaScript.Strategies.CRT_NT
{
    ...
    //======================================================================
    // CRT_Notify — Discord/Telegram webhook layer (HttpClient-based)
    // Ports ORBNSendDiscord, ORBNSendTelegram, queue, drain, dedupe
    //======================================================================
    public class CRT_Notify
    {
        ...
        private readonly Queue<NotifyQueueItem> messageQueue = new Queue<NotifyQueueItem>();
        private readonly HashSet<string> dedupeKeys = new HashSet<string>();
        private readonly object qSync = new object();
        private bool sendInFlight = false;

        private static readonly HttpClient http = new HttpClient();

        public CRT_Notify(string discordWebhookUrl, string telegramBotToken, string telegramChatId,
        ...
        private bool ClaimOnce(string kind, long ticket, string sessionKey)
        {
            ...
            {
                Console.WriteLine(string.Format("[CRT] ORBNClaimOnce blocked duplicate key: {0}", key));
                return false;
            }
            dedupeKeys.Add(key);
            return true;
        ...
        //----------------------------------------------------------------------
        // Drain one item per pulse. The HTTP send runs on a BACKGROUND task —
        // a slow/unreachable webhook must never block the strategy thread.
        //----------------------------------------------------------------------
        public void DrainQueue()
        {
            NotifyQueueItem item;
            lock (qSync)
            {
                if (sendInFlight || messageQueue.Count == 0) return;
            ...
            {
                bool ok = false;
                try { ok = SendDirect(item.Text, item.ColorInt); }
                catch { }
                lock (qSync)
                {
                    sendInFlight = false;
                    if (!ok && item.Retries < 3)
                    {
                        ...
                        var rest = new List<NotifyQueueItem>(messageQueue);
                        messageQueue.Clear();
                        messageQueue.Enqueue(item);
                        foreach (var m in rest) messageQueue.Enqueue(m);
                    }
                    // 3 failed retries → drop silently (webhook misconfigured)
                }
            });
        }
        ...
        private bool SendDiscord(string text, int colorInt)
        {
            ...
            {
                Console.WriteLine(string.Format("[CRT] Discord webhook error: {0}", ex.Message));
                return false;
            }
        }
        ...
        private bool SendTelegram(string text)
        {
            ...
            {
                Console.WriteLine(string.Format("[CRT] Telegram webhook error: {0}", ex.Message));
                return false;
            }
        }
