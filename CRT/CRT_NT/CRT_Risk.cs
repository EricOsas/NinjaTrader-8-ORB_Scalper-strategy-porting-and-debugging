...
// CRT_Risk.cs — lifted VERBATIM from ORB_NT/ORB_Risk.cs (see CRT/02-NT8-IMPLEMENTATION.md §0).
// Risk-% position sizing, margin table, margin cap, Micro/Mini contract caps.
// NOTE: In CRT the SL distance passed to CalcContracts is DYNAMIC (per-setup:
// |entry - manipulationExtreme|), not a fixed input as in ORB.
namespace NinjaTrader.NinjaScript.Strategies.CRT_NT
{
    ...
    public static class CRT_Risk
    {
        ...
        private static readonly Dictionary<string, double> MarginTable = new Dictionary<string, double>()
        {
            ...
            { "YM", 500.0 },
            { "RTY", 500.0 },
            // Metals — GC is $10k+ overnight but typical prop/day margin is much lower;
            // these are conservative intraday figures. Adjust to your broker/firm.
            { "GC", 1000.0 },
            { "MGC", 100.0 },
            { "SI", 1500.0 },
            { "SIL", 150.0 },
        ...
        public static int CalcContracts(double accountBalance, double riskPercent, double slDistancePoints, double pointValue, ContractSizeMode mode, int maxContractsOverride = 0)
        {
            ...
            // Hard cap enforcement. Override (>0) REPLACES the mode cap outright —
            // it can raise it as well as lower it (e.g. GC full-size where the
            // Micro/Mini presets don't apply).
            int hardCap = (mode == ContractSizeMode.Micro) ? MAX_MICRO_CONTRACTS : MAX_MINI_CONTRACTS;
            if (maxContractsOverride > 0)
