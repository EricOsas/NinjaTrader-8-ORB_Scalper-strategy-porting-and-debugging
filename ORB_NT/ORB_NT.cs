...
namespace NinjaTrader.NinjaScript.Strategies.ORB_NT
{
    ...
    public class FastTradeTracker
    {
        ...
        {
            totalTrades++;
            if (holdSeconds < 10.0) fastTrades++;
    ...
    public class ActiveTrade
    {
        ...
        public double TpPrice { get; set; }     // absolute TP level (persisted for adoption)
        public bool TrailingActivated { get; set; }
        public bool BreakevenSet { get; set; }
        public int Contracts { get; set; }
        public string OcoGroup { get; set; }
        public int SlotIndex { get; set; }
        public string SessionKey { get; set; }  // session that produced this trade
        public double SlipTicks { get; set; }   // fill slippage vs intended level, in ticks
        public bool SlipForceClose { get; set; } // excess slip: trailing replaced by close-at-profit
        public bool Adopted { get; set; }       // re-adopted after a strategy/NT restart
        public double LevelPx { get; set; }     // the range level that produced this trade
        public DateTime LevelTime { get; set; } // when that level formed (range end)
        ...
        public double SnapThreshPrice { get; set; }   // activation threshold (price units)
        public double SnapGapPrice { get; set; }      // trail gap (price units)
        public int SnapMinTrailSec { get; set; }      // min seconds in trade before trail activates
        public bool SnapSpreadComp { get; set; }      // compensate gap/BE by entry spread + commission
        public double SnapEntrySpread { get; set; }   // spread captured at fill (price units)
        public double SnapBECostPrice { get; set; }   // commission cushion (price units)
    }
}
