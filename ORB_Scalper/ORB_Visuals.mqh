...
void DrawRangeBox(string sessionKey, datetime timeStart, datetime timeEnd, double priceHigh, double priceLow)
{
    ...
    string rangeLabel;
    if(g_ctxMode == RANGE_DAILY)        rangeLabel = "Daily Range";
    else if(g_ctxMode == RANGE_WEEKLY)  rangeLabel = "Weekly Range";
    else if(g_ctxMode == RANGE_MONTHLY) rangeLabel = "Monthly Range";
    else
    {
        string sessName = (g_ctxSlotIdx == 1) ? "London" : (g_ctxSlotIdx == 2) ? "Asian" : "NY";
        rangeLabel = StringFormat("%s %d-Min Range", sessName, Inp_RangeMinutes);
    }
    string tip = StringFormat("%s | Hi: %.5f | Lo: %.5f", rangeLabel, priceHigh, priceLow);
...
// Inverse of ClearORBVisuals(exceptSlot): deletes ONLY this slot's own tagged
// objects (range/pending boxes "S<slot>_", and exec objects for tickets
// currently tracked under this slot), leaving every other slot's visuals
// untouched. Needed now that up to 4 slots persist independently - a single
// slot's session-key reset must not touch the other three.
void ClearSlotVisuals(int onlySlot)
{
    string pfx = ORBPfx();
    string slotTag = "S" + IntegerToString(onlySlot) + "_";

    string slotTickets[];
    int slotTicketCount = 0;
    ArrayResize(slotTickets, ArraySize(vTrades));
    for(int j = 0; j < ArraySize(vTrades); j++)
    {
        if(vTrades[j].slotIdx == onlySlot)
        {
            slotTickets[slotTicketCount] = IntegerToString(vTrades[j].ticket);
            slotTicketCount++;
        }
    }
    ArrayResize(slotTickets, slotTicketCount);
    ...
    for(int i = ObjectsTotal(0) - 1; i >= 0; i--)
    {
        ...
        bool matchesSlotTag = (StringFind(name, slotTag) >= 0);
        bool matchesSlotTicket = false;
        for(int k = 0; k < slotTicketCount; k++)
        {
            if(StringFind(name, "_" + slotTickets[k]) >= 0) { matchesSlotTicket = true; break; }
        }

        if(matchesSlotTag || matchesSlotTicket)
            ObjectDelete(0, name);
    }
}
