...
string _DB_SessionLabel()
{
    if(g_activeSession == 1 && Inp_EnableLondon) return _DB_LabelForSession(1);
    if(g_activeSession == 2 && Inp_EnableAsian)  return _DB_LabelForSession(2);
    if(g_activeSession == 0 && Inp_EnableNY)     return _DB_LabelForSession(0);
    int enabledCount = (Inp_EnableNY?1:0) + (Inp_EnableLondon?1:0) + (Inp_EnableAsian?1:0);
    if(enabledCount == 1)
    {
        if(Inp_EnableLondon) return _DB_LabelForSession(1);
        if(Inp_EnableAsian)  return _DB_LabelForSession(2);
        return _DB_LabelForSession(0);
    }
    if(Inp_EnableNY)     return _DB_LabelForSession(0);
    if(Inp_EnableLondon) return _DB_LabelForSession(1);
    if(Inp_EnableAsian)  return _DB_LabelForSession(2);
    return "Slot 1 Session:";
}
...
void UpdateDashboard(string status, double rangeHigh, double rangeLow, double rangeRaw, double slPts, double entrySpreadRaw)
{
    ...
    datetime sessionOpen = SessionOpenServer(srv);
    datetime cutoff = SessionCutoffServer(srv);
    string cntdown = (g_activeSession < 0 || cutoff <= sessionOpen) ? "Disabled"
                    : (g_sessionIsLive && srv < cutoff) ? "Open"
                    : (srv >= cutoff) ? "Closed"
    ...
    _DB_Label("ORBD_TIMER",  X+12, Y+68, _DB_SessionLabel() + " " + cntdown, 8, _DB_TEXT());
