...
datetime SessionOpenServer(datetime serverTime)
{
    if(g_ctxMode == RANGE_DAILY)
        return (g_ctxSlotIdx == 1 && Inp_Slot2AnchorMode == 0) ? GetCurrentNYTrueDayOpen(serverTime)
                                                               : GetCurrentD1Open(serverTime);
    if(g_ctxMode == RANGE_WEEKLY)
        return (g_ctxSlotIdx == 1 && Inp_Slot2AnchorMode == 0) ? GetCurrentNYTrueWeekOpen(serverTime)
                                                               : GetCurrentW1Open(serverTime);
    if(g_ctxMode == RANGE_MONTHLY)
        return (g_ctxSlotIdx == 1 && Inp_Slot2AnchorMode == 0) ? GetCurrentNYTrueMonthOpen(serverTime)
