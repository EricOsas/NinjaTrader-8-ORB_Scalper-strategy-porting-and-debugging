...
enum ENUM_SLOT1_MODE
{
    SLOT1_NONE     = 0, // - Disabled -
    SLOT1_INTRADAY = 1, // Intraday sessions only
    SLOT1_MONTHLY  = 2, // Monthly mode overriding Slot 1
    SLOT1_WEEKLY   = 3  // Weekly mode overriding Slot 1
};

enum ENUM_SLOT2_MODE
{
    SLOT2_NONE   = 0, // - Disabled -
    SLOT2_DAILY  = 1, // Daily     - prior D1 candle
    SLOT2_WEEKLY = 2, // Weekly    - prior W1 candle
    SLOT2_MONTHLY= 3  // Monthly   - prior MN1 candle
};
...
input ENUM_SLOT1_MODE   Slot1_Mode               = SLOT1_INTRADAY;    // Slot 1  |  Intraday model
input ENUM_RANGE_MINUTES Slot1_Range_Minutes     = RANGE_15_MIN;      // Slot 1  |  Trigger candle length  (intraday only)

input ENUM_SLOT2_MODE   Slot2_Mode               = SLOT2_NONE;        // Slot 2  |  Secondary range model
input ENUM_SLOT2_ANCHOR Slot2_Anchor_Mode        = SLOT2_ANCHOR_NY_TRUE; // Slot 2  |  Daily/weekly anchor
input int               Slot2_Cutoff_Periods     = 1;                  // Slot 2  |  D1/W1 expiry  (periods)
...
input double            BE_Cost_Points           = 0.0;               // BE offset  (extra points to cover commission)
...
input bool              Use_Spread_Compensation  = true;              // Spread compensation on trail levels
...
input group "   b"
input group "==========  4b - Instant Market Execution  =========="
...
input int               Max_Deviation            = 20;                 // Max order deviation  (points)
...
void SnapshotManagement(int idx)
{
    ...
    vTrades[idx].snapBECostRaw   = PtsToRaw(MathMax(0.0, BE_Cost_Points));
...
string SessionTag()
{
  ...
  if(g_ctxMode == RANGE_MONTHLY) return "MONTHLY";
    if(g_activeSession == 1) return "LDN";
    if(g_activeSession == 2) return "ASIA";
    return "NY";
}
...
string SessionLabel()
{
  ...
  if(g_ctxMode == RANGE_MONTHLY) return "Monthly";
  if(g_ctxMode == RANGE_MONTHLY) return "MONTHLY";
    string sess = (g_activeSession==1) ? "London" : (g_activeSession==2) ? "Asian" : "NY";
...
datetime Slot2OpenAt(datetime serverTime)
{
    ENUM_RANGE_MODE savedMode = g_ctxMode;
    int savedSlot = g_ctxSlotIdx;
    int savedCutoff = g_ctxCutoffMin;
    int savedHour = g_ctxHour;
    int savedMin = g_ctxMin;

    g_ctxSlotIdx = 1;
    if(Slot2_Mode == SLOT2_MONTHLY)
  {
      g_ctxMode = RANGE_MONTHLY;
      g_ctxCutoffMin = Slot2_Cutoff_Periods * 43200; // rough 30 days, true cutoff handled by true month anchor if needed? Actually monthly expiry is exactly 1 month. The CutoffMin is used for SessionCutoffServer. Wait! SessionCutoffServer adds CutoffMin. For Monthly, we should add 43200 minutes or just use exact month.
  }
  else if(Slot2_Mode == SLOT2_WEEKLY)
  {
    ...
    g_ctxMode = savedMode;
    g_ctxSlotIdx = savedSlot;
    g_ctxCutoffMin = savedCutoff;
    g_ctxHour = savedHour;
    g_ctxMin = savedMin;
    return open;
}

datetime Slot2CutoffFromOpen(datetime open)
{
    int minutes = (Slot2_Mode == SLOT2_WEEKLY) ? Slot2_Cutoff_Periods * 10080 : Slot2_Cutoff_Periods * 1440;
    datetime cutoff = open + (datetime)(minutes * 60);
    datetime carry = WeekendCarryCutoffForAnchor(open);
    return (carry > cutoff) ? carry : cutoff;
}

bool FallbackPendingExpired(ulong ticket, datetime now, string &reason)
{
    if(Slot2_Mode == SLOT2_NONE) return false;
    if(!OrderSelect(ticket)) return false;

    string c = OrderGetString(ORDER_COMMENT);
    int parsedSlot = ParseOrderSlot(c);
    if(parsedSlot == 0) return false;
    if(parsedSlot > 1) return false;

    datetime setup = (datetime)OrderGetInteger(ORDER_TIME_SETUP);
    if(setup <= 0) return false;

    datetime currentOpen = Slot2OpenAt(now);
    datetime currentCutoff = Slot2CutoffFromOpen(currentOpen);
    datetime setupCarry = WeekendCarryCutoffForAnchor(setup);
    if(setupCarry > 0 && now < setupCarry) return false;

    if(setup < currentOpen)
    {
        reason = "stale daily/weekly model period";
        return true;
    }
    if(now >= currentCutoff)
    {
        reason = "daily/weekly model cutoff";
        return true;
    }
    return false;
}
...
bool SlotEnabled(int slotIdx)
{
    if(slotIdx == 0)
    {
        if(Slot1_Mode == SLOT1_NONE) return false;
        if(Slot1_Mode == SLOT1_MONTHLY || Slot1_Mode == SLOT1_WEEKLY) return true;   // self-sufficient override, no intraday session required
        return AnyIntradaySessionEnabled();            // SLOT1_INTRADAY
    }
    return (Slot2_Mode != SLOT2_NONE);
}
bool SlotIsIntraday(int slotIdx)
{
    return (slotIdx == 0 && Slot1_Mode == SLOT1_INTRADAY);
}
...
bool ResolveActiveSession()
{
    if(!AnyIntradaySessionEnabled())
    {
        int prevNone = g_activeSession;
        g_activeSession = -1;
        g_sessionIsLive = false;
        Inp_OpenActiveHour = 0;
        Inp_OpenActiveMin = 0;
        Inp_CutoffActiveMin = 0;
        return (prevNone != -1);
    }
    ...
    int prev = g_activeSession;
    int best = -1, next = -1;
    int bestStart = -1, nextStart = 9999;

    for(int s=0; s<3; s++)
    {
        if(!SessionEnabled(s)) continue;

        int openMin = SessionOpenHour(s)*60 + SessionOpenMin(s);
        int endMin  = openMin + SessionCutoffMin(s);
        if(nowMin >= openMin && nowMin < endMin && openMin > bestStart) { best = s; bestStart = openMin; }
        if(openMin > nowMin && openMin < nextStart) { next = s; nextStart = openMin; }
    }

    int chosen;
    if(best >= 0)      { chosen = best; g_sessionIsLive = true; }
    else if(next >= 0) { chosen = next; g_sessionIsLive = false; }
    else               { chosen = (Enable_NY_Session?0:(Enable_London_Session?1:2)); g_sessionIsLive = false; }

    g_activeSession     = chosen;
    Inp_OpenActiveHour  = SessionOpenHour(chosen);
    Inp_OpenActiveMin   = SessionOpenMin(chosen);
    Inp_CutoffActiveMin = SessionCutoffMin(chosen);
    return (chosen != prev);
}
...
void SetSlotContext(int slotIdx)
{
    ...
    if(slotIdx == 0)
    {
        if(Slot1_Mode == SLOT1_MONTHLY)
        {
            // Self-sufficient monthly override for Slot 1 - mirrors Slot 2's
            // MONTHLY case: one period's worth of minutes, non-user-facing,
            // the range is forgotten once the next month's range replaces it.
            g_ctxMode      = RANGE_MONTHLY;
            g_ctxHour      = Inp_OpenActiveHour;
            g_ctxMin       = Inp_OpenActiveMin;
            g_ctxCutoffMin = 43200;
        }
        else if(Slot1_Mode == SLOT1_WEEKLY)
        {
            // Self-sufficient weekly override for Slot 1 - mirrors Slot 2's
            // WEEKLY case.
            g_ctxMode      = RANGE_WEEKLY;
            g_ctxHour      = Inp_OpenActiveHour;
            g_ctxMin       = Inp_OpenActiveMin;
            g_ctxCutoffMin = 10080;
        }
        else
        {
            // Primary slot: disabled/intraday only.
            g_ctxMode      = RANGE_INTRADAY;
            g_ctxHour      = Inp_OpenActiveHour;
            g_ctxMin       = Inp_OpenActiveMin;
            g_ctxCutoffMin = Inp_CutoffActiveMin;
        }
    }
    else
    {
...
int OnInit()
{
    ...
    // Resolve the ACTIVE session chronologically (which session is in-window now),
    // bridging its open/cutoff to the time engine + dashboard. Recomputed live in
    // OnTick so the label and trade engine follow the real current session.
    ...
    trade.SetDeviationInPoints(Max_Deviation);
    ...
    string newSig = StringFormat("RISK%.4f:%d:%.2f:%d:%.2f|GEOM%d:%d:%.6f:%.2f:%.2f|SLT%d|TPD%d:%.2f|S1%d|R%d|S2%d:%d|REV%d|NY%d:%d:%d|LO%d:%d:%d|AS%d:%d:%d",
                                 ...
                                 (int)Slot1_Mode, (int)Slot1_Range_Minutes,
                                 (int)Slot2_Mode, Slot2_Cutoff_Periods, (int)Reverse_Orders,
...
void OnTick()
{
    ...
    // - PRIORITY 2: Session-key reset - initialise slot 0 context first -
    bool activeChanged = ResolveActiveSession();
    SetSlotContext(0);         // populate g_ctx* from slot 0 / primary session
    LoadSlotToGlobals(0);     // restore slot 0 mutable state into globals
    ...
    if(SlotEnabled(0))
    {
        ...
        if(key != g_sessionKey)
        {
            ...
            g_buyPlaced    = false;
            g_sellPlaced   = false;
            ClearORBVisuals(1);   // protect Slot 1 (weekly/monthly) objects from Slot 0's daily reset
        }
    }
    SaveGlobalsToSlot(0);

    // - PRIORITY 2b: Live active-session follow -
    if(activeChanged && SlotEnabled(0))
    {
        LoadSlotToGlobals(0);
        ClearORBVisuals(1);   // protect Slot 1 (weekly/monthly) objects from Slot 0's active-session follow
        g_rangeReady = false;
        g_rangeHigh  = 0.0; g_rangeLow = 0.0;
        g_rangeHighTime = 0; g_rangeLowTime = 0;
        ResetRangeBuild();
        ClearPendingSendLocks();
        g_precloseArmed = false;
        g_buyPlaced  = false;
        ...
        SaveGlobalsToSlot(0);
        if(!Inp_StealthMode)
            ORBLog(StringFormat("[ORB] Active session changed -> %s; fresh range will arm (open positions kept).",
                                (g_activeSession==1)?"London":(g_activeSession==2)?"Asian":"NY"));
    }

    // - PRIORITY 2c: Session-key reset for slot 2 -
    if(SlotEnabled(1))
    {
        SetSlotContext(1);
        LoadSlotToGlobals(1);
        string key2 = ActiveSessionKey(now);
        if(key2 != g_sessionKey)
        {
            g_sessionKey   = key2;
            g_rangeReady   = false;
            g_rangeHigh    = 0.0; g_rangeLow = 0.0;
            ...
            ClearPendingSendLocks();
            g_precloseArmed = false;
            g_buyPlaced = false; g_sellPlaced = false;
            ClearORBVisuals(0);   // protect Slot 0 (intraday/daily) objects from Slot 1's reset
        }
        SaveGlobalsToSlot(1);
        // Restore slot 0 context so Priority 3 cutoff check is for slot 0
        SetSlotContext(0);
        LoadSlotToGlobals(0);
    }

    // CANCEL_PENDING during news blackout (once per session when window opens).
    if(ORBNNewsEnabled() && News_Cancel_Pending)
    {
        static string s_newsCancelKey = "";
        string nck = g_sessionKey + ":NC";
        if(IsNewsBlocked(now) && s_newsCancelKey != nck)
        {
            s_newsCancelKey = nck;
            CancelPendingOrders();
    ...
    // - PRIORITY 3: Slot 0 cutoff - if BOTH slots past cutoff, return early -
    static string s_cutoffDoneKey = "";
    datetime cutoff = EffectiveSlotCutoffServer(now);  // uses slot 0 context
    bool slot0Past = SlotEnabled(0) && (now >= cutoff);
    if(slot0Past)
    {
        string ck = g_sessionKey + ":" + IntegerToString(g_activeSession);
        if(s_cutoffDoneKey != ck)
        {
            s_cutoffDoneKey = ck;
        ...
        // Only return early if slot 2 is also past cutoff (or disabled)
        bool slot2Past = true;
        if(SlotEnabled(1))
        {
            SetSlotContext(1);
            LoadSlotToGlobals(1);
            slot2Past = (now >= EffectiveSlotCutoffServer(now));
            // Restore slot 0 context
            SetSlotContext(0); LoadSlotToGlobals(0);
        }
        if(slot2Past) return;
    }
    ...
    for(int s = 0; s < 2; s++)
    {
        ...
        if(now >= slotCutoff) { SaveGlobalsToSlot(s); continue; }
        ...
        if(g_rangeReady && (!g_buyPlaced || !g_sellPlaced))
        {
            ...
            {
                static string s_newsBlockLogKey[2] = {"", ""};
                int nbSlot = (s >= 0 && s < 2) ? s : 0;
    ...
    int ltfSec = (int)Slot1_Range_Minutes * 60;
    datetime curBar = (ltfSec > 0) ? (datetime)((long)now / ltfSec * ltfSec) : now;
...
void NormalizeStopsToFill(ulong ticket, double fillPx, double levelPx)
{
    ...
    double slipPts = MathAbs(fillPx - levelPx) / _Point;
    ...
    if(Max_Slippage_Points > 0 && slipPts > Max_Slippage_Points)
    {
...
void RegisterNewPositions(datetime now)
{
    for(int i = PositionsTotal()-1; i >= 0; i--)
    {
        ...
        double   levelPx   = hasMeta ? meta.levelPx   : (bull ? g_rangeHigh : g_rangeLow);
        datetime levelTime = hasMeta ? meta.levelTime : (bull ? g_rangeHighTime : g_rangeLowTime);
        int      slotIdx   = hasMeta ? meta.slotIdx   : -1;
...
void RedrawBrokerExecutions()
{
    ...
    if(SlotEnabled(1))
    {
        if(Slot2_Mode == SLOT2_MONTHLY) scanFrom = MathMin(scanFrom, (Inp_Slot2AnchorMode == 0) ? GetCurrentNYTrueMonthOpen(now) : GetCurrentMN1Open(now));
      else if(Slot2_Mode == SLOT2_WEEKLY) scanFrom = MathMin(scanFrom, s2Wk);
      else                           scanFrom = MathMin(scanFrom, s2Day);
    }
    ...
    for(int d = 0; d < total; d++)
    {
        ...
        if(slotIdx == 1)
        {
            if(Slot2_Mode == SLOT2_MONTHLY) slotStart = (Inp_Slot2AnchorMode == 0) ? GetCurrentNYTrueMonthOpen(now) : GetCurrentMN1Open(now);
          else if(Slot2_Mode == SLOT2_WEEKLY) slotStart = s2Wk;
          else                           slotStart = s2Day;
        }
...
void SimulateSyntheticExecutions()
{
    ...
    for(int s = 0; s < 2; s++)
    {
        ...
        // Slot 1 in Weekly/Monthly override mode and Slot 2 set to the same
        // period is the identical range/level - only monitor it once (via
        // Slot 1), instead of placing two independent watchers on one range.
        if(s == 1 && ((Slot1_Mode == SLOT1_WEEKLY  && Slot2_Mode == SLOT2_WEEKLY) ||
                      (Slot1_Mode == SLOT1_MONTHLY && Slot2_Mode == SLOT2_MONTHLY)))
            continue;
        ...
        int mode = (s == 0) ? (int)Slot1_Mode : (int)Slot2_Mode;
...
void ManageTrailingStops()
{
    ...
    for(int i = 0; i < ArraySize(vTrades); i++)
    {
        ...
        if(vTrades[i].snapBehavior == TRAILB_BE)
        {
            ...
            double cost = spread + vTrades[i].snapBECostRaw;
...
void CheckOpeningRange(datetime now)
{
    ...
    if(g_ctxMode != RANGE_INTRADAY)
    {
        ...
        if(now >= sessionOpen)
        {
            ...
            bool trueAnchor = (g_ctxSlotIdx == 1 && Inp_Slot2AnchorMode == 0);
    ...
    else
    {
        if(!LoadRangeWindow(rangeStart, rangeEnd, hi, lo, hiTime, loTime))
        {
            ...
            if(now < fallbackEnd)
            {
                static string s_fallbackWaitLogKey = "";
                if(s_fallbackWaitLogKey != g_sessionKey)
                {
                    s_fallbackWaitLogKey = g_sessionKey;
                    ORBLog(StringFormat("[ORB] Pre-open range unavailable for %s; waiting for post-open fallback range.",
                                        (g_activeSession==2)?"Asian":(g_activeSession==1)?"London":"NY"));
                }
                return;
            }
            if(!LoadRangeWindow(fallbackStart, fallbackEnd, hi, lo, hiTime, loTime))
            {
                static string s_rangeCopyFailLogKey = "";
                if(s_rangeCopyFailLogKey != g_sessionKey)
                {
                    s_rangeCopyFailLogKey = g_sessionKey;
...
//+------------------------------------------------------------------+
static string s_instFailKey[2]       = {"", ""};
static int    s_instFailCountHigh[2] = {0, 0};
static int    s_instFailCountLow[2]  = {0, 0};

void ResetInstantFailCountersIfNewSession()
{
    int slotIdx = (g_ctxSlotIdx >= 0 && g_ctxSlotIdx < 2) ? g_ctxSlotIdx : 0;
...
ENUM_LEVEL_CROSS ClassifyLevelCrossing(bool isHigh, double levelPx, datetime &crossTime)
{
    ...
    int slotIdx = (g_ctxSlotIdx >= 0 && g_ctxSlotIdx < 2) ? g_ctxSlotIdx : 0;
    static string   s_sessH[2] = {"", ""};
    static string   s_sessL[2] = {"", ""};
    static datetime s_curH[2]  = {0, 0};
    static datetime s_curL[2]  = {0, 0};
    static bool     s_staleDoneH[2] = {false, false};
    static bool     s_staleDoneL[2] = {false, false};
...
void ExecuteInstantSide(bool isHigh, double slRaw)
{
    ...
    if(sent && (trade.ResultRetcode() == TRADE_RETCODE_DONE || trade.ResultRetcode() == TRADE_RETCODE_DONE_PARTIAL))
    {
        ...
        // Max-slippage check (from level to actual fill)
        double slipFill = MathAbs(actualFillPx - levelPx) / _Point;
        ulong posForSlip = trade.ResultOrder();
        if(Max_Slippage_Points > 0 && slipFill > Max_Slippage_Points && posForSlip > 0)
        {
            GlobalVariableSet(StringFormat("ORB_SLIPARM_%I64u", posForSlip), 1.0);
            ORBLog(StringFormat("[ORB] Ticket #%I64u slip %.1f pts > Max_Slippage_Points=%.1f - trailing disabled; close at +%.1f pts profit.",
                                posForSlip, slipFill, Max_Slippage_Points, Slip_ForceClose_Profit_Points));
        }
    ...
    else
    {
        ...
        else
        {
            ...
            int fcSlot = (g_ctxSlotIdx >= 0 && g_ctxSlotIdx < 2) ? g_ctxSlotIdx : 0;
...
void PlaceOrders(datetime now, datetime cutoff)
{
    ...
    if(!BrokerAllowsNewEntries())
    {
        ...
        static datetime s_brokerPauseLast[2] = {0, 0};
        int bpSlot = (g_ctxSlotIdx >= 0 && g_ctxSlotIdx < 2) ? g_ctxSlotIdx : 0;
    ...
    static string s_failKey[2] = {"", ""};
    static int s_failCountHigh[2] = {0, 0};
    static int s_failCountLow[2] = {0, 0};
    int fcSlotStop = (g_ctxSlotIdx >= 0 && g_ctxSlotIdx < 2) ? g_ctxSlotIdx : 0;
...
void UpdateVisuals()
{
    ...
    for(int i = 0; i < ArraySize(vTrades); i++)
    {
        ...
        if(vTrades[i].slotIdx != g_ctxSlotIdx) continue;
